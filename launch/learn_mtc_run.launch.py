...
def generate_launch_description():
    ...
    learn_mtc_params_file = os.path.join(
        get_package_share_directory('learn_mtc'), 'config', 'learn_mtc_run_config.yaml'
    )
    ...
    # 5. Gazebo 환경 초기화 스크립트
    reset_script = os.path.join(
        get_package_share_directory('learn_mtc'), 'scripts', 'reset_gz_world.py'
    )
    reset_gz_world = ExecuteProcess(
        cmd=['python3', reset_script],
        output='screen',
    )

    # 6. MTC 노드를 리셋 완료 후 2초 뒤 실행
    ...
    # 선언된 인자들과 노드를 함께 리턴
    return LaunchDescription(parameter_arguments + [reset_gz_world, delayed_pick_place])